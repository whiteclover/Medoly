...
"""An implementation of the Event Process Bus.


Event Process Bus
--------------------

A Bus object is used to contain and manage event task.
...
class Horae(object):
    ...
    def subscribe(self, channel, callback, priority=None):
        ...
        self.listeners[channel][:] = self._sorted_items(channel)
    ...
    def _sorted_items(self, channel):
    ...
    def publish(self, channel, aysnc, *args, **kwargs):
        """Return output of all subscribers for the given channel."""
        if channel not in self.listeners:
            return []
        items = self.listeners[channel]
        if async:
            pros = Pros(channel, items, args, kwargs)
            self.executor.put(pros)
        else:
            return self._sync_run(channel, items, args, kwargs)

    def _sync_run(self, channel, items):
        exc = ChannelFailures()
        output = []
    ...
    def log(self, msg="", level=20, traceback=False):
        ...
        self.publish('log', False, msg, level)
